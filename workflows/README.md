...
| `named-workers-v10.json` | … (named workers) v10 | 39 · 5 | 2026-09-26 | `namedworkersv10` |
| `named-workers-v9-claude.json` | … (named workers) v9-claude | 36 · 5 | 2026-09-25 | `namedworkersv9claude` |
...
★ = main, the one to reference and build on. v10 = v9-claude plus a worker Exit check, a Quality gate cell (tests pass, commit tagged, checkpoint written, how-to-verify done; Enforced by: self, so self-checked until a Codex Stop hook is added) before a result is sent, and a Handoff check Quality gate inside ② Compact or hand over (5 sections filled, failing tests listed, doc committed, git status clean) before a handoff doc is sent, looping back to a Step cell "Write / fix handoff doc" (done by the worker) that writes the Handoff doc memory cell; fix first if a check fails, send as BLOCKED if stuck; orchestrator caps fix rounds at 2 per task. v9-claude = Claude's recommended take on v8 (not replacing it): both threads use the same loop (Agent → Context check → ① Safe point → ② Compact or hand over → back), one colour meaning everywhere, short box text with detail in the Details panel, progress.md as the single source of truth, and subagents for noisy work. v8 = v7 reorganised: on the Orchestrator Thread page, the safe-point, worker-pause and commit steps are one sub-flow, "Reach a safe point (pause any active worker)", followed by a second, "Compact or hand over"; the old "Context full" box is gone. v7 = v6 plus a worker check in the orchestrator's Context full: at the safe point, if a worker is mid-task the orchestrator tells it to save and pause, waits for its reply, and tells it to resume after compacting (after a handover, the new orchestrator does). v6 = v5 with the orchestrator handover folded into one place: Fresh orchestrator and Handover check now live inside the Orchestrator Thread's "Context full: compact or hand over", which has two exits back (continue compacted, O02 takes over). v5 = v4 plus a "Handover check" sub-flow: old and new orchestrator overlap briefly, run a 3-point check, then ownership (OWNER line in progress.md) moves to the new one. v4 = v3 with the Orchestrator turned into an "Orchestrator Thread" sub-flow that follows the same compact-once-then-hand-off rule as workers. v3 = v2 with compaction merged into the handoff sub-flow (now "Context full: compact or hand off"). "Boxes" counts boxes on every level; "links" counts only the top level.
...
- A **Quality gate** is a box with `"type": "gate"`: `checks` lists what must all pass, and `enforce` says who makes sure (`self` = the agent checks its own work, nothing outside enforces it; `hook`, `script`, `orchestrator` or `human` = something outside does). When writing an AGENTS.md, a `self` gate becomes a rule; a `hook` or `script` gate should also become that hook or script.
- A **Step** is a box with `"type": "step"`: an action, and `by` says who does it (`worker`, `orchestrator`, `subagent`, `user`). Memory, Folder and Tool boxes are things, not actions; loops go back to a Step or an Agent.
