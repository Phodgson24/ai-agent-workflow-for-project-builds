...
- The repo file `app/agent-flow-sketchpad.html` is the source of truth. It matches the live artifact as last published from this repo (version 28, Computer OS for folders).
...
- v10: `namedworkersv10` (37 boxes · 5 links). v9-claude copy plus a worker "Exit check" gate (`wexit`, Worker Thread): task finished → Exit check → all pass → result / a check fails → fix first / stuck → send as BLOCKED. Worker rule explains why (cheaper than review round trips, which fill the orchestrator's context). Orchestrator rules: max 2 fix rounds per task, then tell the user; handle BLOCKED results. Based on OpenAI's "Build iterative repair loops with Codex" stop conditions.
