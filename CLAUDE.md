...
## Setups (app database)
- **Main setup:** `namedworkersv8`, "user agent + project folder (named workers) v8" (36 boxes · 5 links). v7 copy where the 5 safe-point boxes (At a safe point?, Finish the current step, Worker active?, Tell the worker: save and pause, Worker paused?) moved into a sub-flow `sp`, "Reach a safe point (pause any active worker)", inside the orchestrator's Context full (in: `h1` blue, out: `spout` violet → Commit progress.md).
