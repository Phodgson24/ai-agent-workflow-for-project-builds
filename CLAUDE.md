...
## Setups (app database)
- **Main setup:** `namedworkersv8`, "user agent + project folder (named workers) v8" (36 boxes · 5 links). v7 copy where the 6 safe-point boxes (At a safe point?, Finish the current step, Worker active?, Tell the worker: save and pause, Worker paused?, Commit progress.md) moved into a sub-flow `sp`, "Reach a safe point (pause any active worker)", inside the orchestrator's Context full (in: `h1` blue, out: `spout` violet). The rest (Compacted yet?, Compact, Tell the worker to resume, Write handoff doc, Fresh orchestrator, Handover check) is in a second sub-flow `ch`, "Compact or hand over" (exits `chc` red → continue, `cht` pink → O02 takes over). Context full is now just those two boxes.
