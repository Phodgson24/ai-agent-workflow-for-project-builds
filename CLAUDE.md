...
- The repo file `app/agent-flow-sketchpad.html` is the source of truth. It matches the live artifact as last published from this repo (version 30, Step cell).
...
## Setups (app database)
- **Main setup:** `namedworkersv8`, "user agent + project folder (named workers) v8" (36 boxes · 5 links). v7 copy, reorganised on the Orchestrator Thread page: Current orchestrator → Context check → sub-flow `sp` "Reach a safe point (pause any active worker)" (At a safe point?, Finish the current step, Worker active?, Tell the worker: save and pause, Worker paused?, Commit progress.md) → sub-flow `ch` "Compact or hand over" (Compacted yet?, Compact, Tell the worker to resume, Write handoff doc, Fresh orchestrator, Handover check), which returns to the orchestrator via `chc` (red, continue) and `cht` (pink, O02 takes over). Links: `h1` (orange, in), `spout` (cyan). The old "Context full: compact or hand over" box was emptied and deleted.
- v10: `namedworkersv10` (39 boxes · 5 links). v9-claude copy plus a worker "Exit check" (`wexit`, Worker Thread), a **Quality gate** cell (4 checks, Enforced by: self): task finished → Exit check → all pass → result / a check fails → fix first / stuck → send as BLOCKED. Worker rule explains why (cheaper than review round trips, which fill the orchestrator's context). Orchestrator rules: max 2 fix rounds per task, then tell the user; handle BLOCKED results. Based on OpenAI's "Build iterative repair loops with Codex" stop conditions. Inside the worker's ② Compact or hand over: `hd` Step "Write / fix handoff doc" (done by worker; writes `hdoc`, a Memory cell "Handoff doc", via `d8`; commits it) → `hck` "Handoff check" Quality gate (5 sections, failing tests listed under Open issues, doc committed, git status clean; enforced by self) → all pass → send (`d6`) / a check fails → fix the doc (`d7`). A handover skips the Exit check (partly done work). Fresh worker rule: tell the orchestrator if the handoff doc is missing something.
- v9-claude: `namedworkersv9claude` (36 boxes · 5 links). Claude's recommended version of v8, built on request as an alternative; v8 stays the user's main. Same loop on both thread pages (the user's v8 clockwise layout), colour legend in canvas notes, short text, recommendations from official docs (durable memory in progress.md, subagents for noisy work, goal-style task hand-outs).
- v7: `namedworkersv7`, "user agent + project folder (named workers) v7" (35 boxes · 5 links). v6 copy plus a worker check in the orchestrator's Context full: At a safe point? → Worker active? → (yes) Tell the worker: save and pause → Worker paused? → Commit; after compacting, Tell the worker to resume. The user's request ended at "and once…"; the resume step is Claude's completion, flagged to the user.
