...
## Setups (app database)
- **Main setup:** `namedworkersv8`, "user agent + project folder (named workers) v8" (35 boxes · 5 links). v7 copy, reorganised on the Orchestrator Thread page: Current orchestrator → Context check → sub-flow `sp` "Reach a safe point (pause any active worker)" (At a safe point?, Finish the current step, Worker active?, Tell the worker: save and pause, Worker paused?, Commit progress.md) → sub-flow `ch` "Compact or hand over" (Compacted yet?, Compact, Tell the worker to resume, Write handoff doc, Fresh orchestrator, Handover check), which returns to the orchestrator via `chc` (red, continue) and `cht` (pink, O02 takes over). Links: `h1` (orange, in), `spout` (cyan). The old "Context full: compact or hand over" box was emptied and deleted.
